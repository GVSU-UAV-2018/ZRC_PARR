from zrc_base import msg_id_to_type
import time
import threading
...
class GroundRadioFinder(object):
    def __init__(self, *args, **kwargs):
        ...
        self.msg_read_alive = threading.Event()
        self.msg_read_alive.set()
        self.msg_read_thread = threading.Thread(target=self._handle_msg)
    ...
    def update_scan_settings(self):
        self.serial_port.send_scan_settings(self.gain,
                                            self.scan_frequency,
                                            self.snr_threshold)
    ...
    def _handle_msg(self):
        while self.msg_read_alive:
            msg = self.serial_port.in_q.get(block=True, timeout=0.1)
            if msg is not None:
                if msg_id_to_type[msg.msg_id] == 'attitude':
                    pub.sendMessage('msg_attitude', msg.altitude, msg.heading)
                elif msg_id_to_type[msg.msg_id] == 'detection':
                    pub.sendMessage('msg_detection', msg.snr, msg.heading)
        


    def _send_scanning(self):
        while self.scan_alive:
            self.serial_port.send_scanning(self.scanning)
            time.sleep(0.5)
    ...
    def start(self):
        ...
        self.msg_read_thread.start()

    def close(self):
        self.scan_alive.clear()
        self.msg_read_alive.clear()
        self.scan_thread.join(timeout=0.5)
        self.msg_read_thread.join(timeout=0.5)
